...
extension MIDIScoreParser {
    ...
    private func getMusicParts(midi: MidiData) -> [MusicPart] {
        ...
        let bpms = tempoParser.getBPMs(midi: midi)
        let signatures = tempoParser.getTimeSignatures(midi: midi)
        ...
        for idx in 0..<midi.noteTracks.count {
            ...
            let notes = getNotes(tempos: tempoChangeLogs, noteTrack: noteTrack)
    ...
    private func getTempo(ts: MusicTimeStampOfQuarters, with tempos: [TempoChangeLog]) -> Tempo? {
        for tempo in tempos {
